    ...
    Zip::File.open(file) do |zip_file|
      zip_file.first(quantity).each_with_index do |entry, index|
        if entry.name.include?("acteur")
          ...
          deputy[:party] = data["acteur"]["mandats"]["mandat"].select{|mandat|mandat["typeOrgane"]=="PARPOL"}.empty? ? "Parti non renseigné" : @parties[data["acteur"]["mandats"]["mandat"].select{|mandat|mandat["typeOrgane"]=="PARPOL"}.first["organes"]["organeRef"]]
          ...
          deputy[:img] = "https://www2.assemblee-nationale.fr/static/tribun/15/photos/#{data['acteur']['uid']['#text'][2..-1]}.jpg"
          deputies << deputy
